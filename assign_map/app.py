...
ACTIVE_MODULE_DIR = MODULE_DIR / "07.06"
if ACTIVE_MODULE_DIR.exists():
    sys.path.insert(0, str(ACTIVE_MODULE_DIR))

from preprocess_blocks import build_processed_payload
from preprocess_transport_blocks import build_payload as build_assignment_block_payload
...
ACTIVE_MAP_DIR = BASE_DIR / "07.06" if (BASE_DIR / "07.06" / "daejeon_map.html").exists() else BASE_DIR


def first_existing_path(*names: str) -> Path:
    for root in (ACTIVE_MAP_DIR, BASE_DIR):
        for name in names:
            path = root / name
            if path.exists():
                return path
    return ACTIVE_MAP_DIR / names[0]


PROCESS_GEOCODE_PATH = first_existing_path(
    "process_geocode_owner_test.json",
    "process_geocode.json",
)
PROCESSED_BLOCKS_TMP_PATH = BASE_DIR / "_processed_blocks_tmp.json"
PROCESSED_ASSIGNMENT_BLOCKS_PATH = first_existing_path("processed_assignment_blocks.json")
PARCEL_SHP_PATH = BASE_DIR / "block_geocodes" / "LSMD_CONT_LDREG_30_202607.shp"
TRANSPORT_SHP_PATH = BASE_DIR / "transport" / "AL_D142_00_20260609.shp"
ROAD_SHP_PATH = BASE_DIR / "road" / "C_UQ151.shp"
DONG_SHP_PATH = BASE_DIR / "dong" / "LSMD_ADM_SECT_UMD_30_202606.shp"
MAP_HTML_PATH = first_existing_path(
    "daejeon_map_owner_tuned.html",
    "daejeon_map.html",
    "safety_single_house_map.html",
)
...
def aggregate_scope_rows(
    ...
    for record_id, ((month, center, address), group) in enumerate(groups, start=1):
        ...
        hosu_values = ordered_unique(group["호수"])
...
def run_data_processing_pipeline() -> dict[str, Any]:
    ...
    # 기존 process_geocode.json 데이터가 있으면 불러와서 병합
    existing_months = []
    existing_centers = []
    existing_scopes = {}
    
    if PROCESS_GEOCODE_PATH.exists():
        try:
            existing_payload = json.loads(PROCESS_GEOCODE_PATH.read_text(encoding="utf-8"))
    ...
    # process_geocode.json 파일 생성
    PROCESS_GEOCODE_PATH.write_text(json.dumps(final_payload, ensure_ascii=False, indent=2), encoding="utf-8")

    missing_inputs = [
        str(path)
        for path in (PARCEL_SHP_PATH, TRANSPORT_SHP_PATH, ROAD_SHP_PATH, DONG_SHP_PATH)
        if not path.exists()
    ]
    if missing_inputs:
        raise ValueError("Required block boundary source files are missing: " + ", ".join(missing_inputs))

    build_processed_payload(
        PROCESS_GEOCODE_PATH.resolve(),
        PARCEL_SHP_PATH.resolve(),
        PROCESSED_BLOCKS_TMP_PATH.resolve(),
        pretty=False,
    )
    try:
        build_assignment_block_payload(
            PROCESSED_BLOCKS_TMP_PATH.resolve(),
            TRANSPORT_SHP_PATH.resolve(),
            ROAD_SHP_PATH.resolve(),
            DONG_SHP_PATH.resolve(),
            PROCESSED_ASSIGNMENT_BLOCKS_PATH.resolve(),
            pretty=False,
        )
    finally:
        try:
            PROCESSED_BLOCKS_TMP_PATH.unlink()
    ...
    return {
        ...
        "success_count": success_count,
        "failed_count": failed_count,
        "assignment_blocks_file": PROCESSED_ASSIGNMENT_BLOCKS_PATH.name,
    }
...
def get_admin_page(response: Response):
    ...
    admin_path = next(
        (
            path
            for path in (BASE_DIR / "admin" / "admin.html", BASE_DIR / "admin.html")
            if path.exists()
        ),
        BASE_DIR / "admin" / "admin.html",
    )
    if not admin_path.exists():
        raise HTTPException(status_code=404, detail="admin/admin.html를 찾을 수 없습니다.")
    return admin_path.read_text(encoding="utf-8")


@app.get("/processed_assignment_blocks.json")
def get_processed_assignment_blocks(response: Response):
    response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
    response.headers["Pragma"] = "no-cache"
    response.headers["Expires"] = "0"
    if not PROCESSED_ASSIGNMENT_BLOCKS_PATH.exists():
        return JSONResponse(status_code=404, content={"error": "processed_assignment_blocks.json not found. Run preprocess_transport_blocks.py first."})
    return FileResponse(PROCESSED_ASSIGNMENT_BLOCKS_PATH, media_type="application/json")
