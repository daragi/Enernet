(() => {
  ...
  const elements = {
    ...
    directErrorOpen: document.querySelector("#direct-error-open"),
    directErrorModal: document.querySelector("#direct-error-modal"),
    directErrorClose: document.querySelector("#direct-error-close"),
    directErrorCancel: document.querySelector("#direct-error-cancel"),
    directErrorPaste: document.querySelector("#direct-error-paste"),
    directErrorPreview: document.querySelector("#direct-error-preview"),
    directErrorPreviewSummary: document.querySelector("#direct-error-preview-summary"),
    directErrorPreviewMessage: document.querySelector("#direct-error-preview-message"),
    directErrorPreviewList: document.querySelector("#direct-error-preview-list"),
    directErrorApprove: document.querySelector("#direct-error-approve"),
  ...
  const hideUpdateAlert = () => {
    ...
    window.clearTimeout(updateAlertFadeTimer);
    if (!elements.updateAlert || elements.updateAlert.hidden) return;
    elements.updateAlert.classList.add("is-fading");
    updateAlertFadeTimer = window.setTimeout(() => {
      elements.updateAlert.hidden = true;
      elements.updateAlert.classList.remove("is-fading");
      document.body.classList.remove("has-update-alert");
    }, 260);
  };
  ...
  const setUpdateAlert = (message, visible = true) => {
    ...
    document.body.classList.add("has-update-alert");
    updateAlertTimer = window.setTimeout(hideUpdateAlert, 5000);
  };
  ...
  const updateToolbarState = () => {
    ...
    elements.directErrorOpen.disabled = !state.jobId || state.mutationInFlight;
  ...
  const setApprovalBusy = (busy) => {
    ...
    elements.directErrorOpen.disabled = busy || !state.jobId;
    elements.directErrorApprove.disabled = busy
      || !state.directError.preview
      || Number(state.directError.preview.ready_count) <= 0
      || (state.directError.preview.missing_order_numbers || []).length > 0
      || (state.directError.preview.already_confirmed_order_numbers || []).length > 0
      || (state.directError.preview.duplicate_order_numbers || []).length > 0;
  ...
  const directErrorCanApprove = (preview = state.directError.preview) => Boolean(
    preview
    && Number(preview.ready_count) > 0
    && !(preview.missing_order_numbers || []).length
    && !(preview.already_confirmed_order_numbers || []).length
    && !(preview.duplicate_order_numbers || []).length
  );

  const clearDirectErrorPreview = () => {
    state.directError.preview = null;
    elements.directErrorPreview.hidden = true;
    elements.directErrorPreview.classList.remove("is-blocked");
    elements.directErrorPreviewSummary.textContent = "";
    elements.directErrorPreviewMessage.textContent = "";
    elements.directErrorPreviewList.replaceChildren();
    elements.directErrorApprove.disabled = true;
  };

  const closeDirectErrorModal = () => {
    window.clearTimeout(state.directError.previewTimer);
    state.directError.requestId += 1;
    elements.directErrorModal.hidden = true;
    elements.directErrorPaste.value = "";
    clearDirectErrorPreview();
  };

  const renderDirectErrorPreview = (preview) => {
    state.directError.preview = preview;
    const total = Number(preview.total_count) || 0;
    const ready = Number(preview.ready_count) || 0;
    const missing = Array.isArray(preview.missing_order_numbers)
      ? preview.missing_order_numbers : [];
    const confirmed = Array.isArray(preview.already_confirmed_order_numbers)
      ? preview.already_confirmed_order_numbers : [];
    const duplicates = Array.isArray(preview.duplicate_order_numbers)
      ? preview.duplicate_order_numbers : [];
    const restore = Array.isArray(preview.excluded_order_numbers)
      ? preview.excluded_order_numbers : [];
    const blocked = missing.length > 0 || confirmed.length > 0 || duplicates.length > 0;
    const messages = [];
    if (missing.length) messages.push(`현재 업로드에서 찾지 못함 ${missing.length}건`);
    if (confirmed.length) messages.push(`이미 오생성 ${confirmed.length}건`);
    if (duplicates.length) messages.push(`붙여넣기 중복 ${duplicates.length}건`);
    if (restore.length) messages.push(`제외 해제 후 재확정 ${restore.length}건`);

    elements.directErrorPreview.hidden = false;
    elements.directErrorPreview.classList.toggle("is-blocked", blocked);
    elements.directErrorPreviewSummary.textContent = blocked
      ? `${numberFormat.format(total)}건을 확인했습니다. 수정 후 다시 붙여 넣어 주세요.`
      : `${numberFormat.format(ready)}건을 오생성으로 등록할 수 있습니다.`;
    elements.directErrorPreviewMessage.textContent = messages.length
      ? messages.join(" · ")
      : "현재 업로드된 전처리 행으로 집계와 대시보드에 즉시 반영됩니다.";
    elements.directErrorPreviewList.replaceChildren();
    (Array.isArray(preview.rows) ? preview.rows : []).forEach((row) => {
      const item = document.createElement("div");
      item.className = "direct-error-preview-row";
      const stateText = textValue(row.상태);
      item.classList.toggle("is-blocked", stateText === "현재 업로드에서 찾지 못함" || stateText === "이미 오생성");
      [
        textValue(row.오더번호),
        textValue(row.소분류),
        textValue(row.내역),
      ...
      item.append(status);
      elements.directErrorPreviewList.append(item);
    });
    elements.directErrorApprove.disabled = !directErrorCanApprove();
  };

  const previewDirectErrorRows = async () => {
    const pastedText = elements.directErrorPaste.value;
    if (!pastedText.trim() || !state.jobId) {
      clearDirectErrorPreview();
      return;
    }
    const requestId = ++state.directError.requestId;
    elements.directErrorPreview.hidden = false;
    elements.directErrorPreview.classList.remove("is-blocked");
    elements.directErrorPreviewSummary.textContent = "붙여넣은 행을 현재 업로드 데이터에서 찾고 있습니다.";
    elements.directErrorPreviewMessage.textContent = "";
    elements.directErrorPreviewList.replaceChildren();
    elements.directErrorApprove.disabled = true;
    try {
      const response = await fetch("/api/admin/direct-errors/preview", {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({ job_id: state.jobId, pasted_text: pastedText }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(responseError(payload, response.status));
      if (requestId !== state.directError.requestId) return;
      renderDirectErrorPreview(payload);
    } catch (error) {
      if (requestId !== state.directError.requestId) return;
      state.directError.preview = null;
      elements.directErrorPreview.hidden = false;
      elements.directErrorPreview.classList.add("is-blocked");
      elements.directErrorPreviewSummary.textContent = "등록할 수 없습니다.";
      elements.directErrorPreviewMessage.textContent = error.message || "붙여넣은 행을 확인하지 못했습니다.";
      elements.directErrorPreviewList.replaceChildren();
      elements.directErrorApprove.disabled = true;
    }
  };

  const openDirectErrorModal = () => {
    if (!state.jobId || state.mutationInFlight) return;
    elements.directErrorModal.hidden = false;
    elements.directErrorPaste.focus();
  };

  const approveDirectErrorRows = async () => {
    if (!state.jobId || !directErrorCanApprove()) return;
    hideToast();
    setApprovalBusy(true);
    try {
      const response = await fetch("/api/admin/direct-errors/approve", {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({
          job_id: state.jobId,
          pasted_text: elements.directErrorPaste.value,
        }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(responseError(payload, response.status));
      state.canRollback = true;
      closeDirectErrorModal();
      ...
      publishDashboardRefresh("direct_error_registered", payload);
      showToast(payload.notification?.message || "원본 행을 오생성으로 반영했습니다.");
    } catch (error) {
      showToast(error.message || "원본 행을 오생성으로 등록하지 못했습니다.");
    } finally {
  ...
  elements.updateClose?.addEventListener("click", hideUpdateAlert);
  ...
  elements.directErrorOpen?.addEventListener("click", openDirectErrorModal);
  elements.directErrorClose?.addEventListener("click", closeDirectErrorModal);
  elements.directErrorCancel?.addEventListener("click", closeDirectErrorModal);
  elements.directErrorPaste?.addEventListener("input", () => {
    window.clearTimeout(state.directError.previewTimer);
    state.directError.preview = null;
    elements.directErrorApprove.disabled = true;
    state.directError.previewTimer = window.setTimeout(previewDirectErrorRows, 180);
  });
  elements.directErrorApprove?.addEventListener("click", approveDirectErrorRows);
  elements.directErrorModal?.addEventListener("click", (event) => {
    if (event.target === elements.directErrorModal) closeDirectErrorModal();
  });
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !elements.directErrorModal?.hidden) {
      closeDirectErrorModal();
    }
  });
