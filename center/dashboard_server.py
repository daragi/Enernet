...
from service_order.historical_loader import (
    load_historical_aggregates,
    load_historical_error_details,
)
...
def cached_historical_result() -> dict[str, object] | None:
    ...
    try:
        ...
        if not isinstance(stored_signature, dict) or (
            current_signature.get("files") != stored_signature.get("files")
        ):
            return None
        if state_version == HISTORICAL_IMPORT_VERSION - 1:
            # v2 retained only aggregate counts. Add the small 1~6월 error
            # detail list without needlessly reading two million source rows.
            details = load_historical_error_details(
                HISTORICAL_TRUTH_PATH,
                HISTORICAL_EXCEPT_LIST_PATH,
                months=range(1, 7),
            )
            expected_count = int(result.get("summary", {}).get("truth_count", -1))
            if len(details) != expected_count:
                return None
            result = deepcopy(result)
            result["historical_details"] = (
                DASHBOARD_STORE.replace_historical_error_details(
                    details,
                    source_name=HISTORICAL_TRUTH_PATH.name,
                )
            )
            save_historical_import_state(result)
        historical_details = result.get("historical_details")
        if not isinstance(historical_details, dict):
            return None
        current_details = DASHBOARD_STORE.historical_error_detail_status()
        if int(current_details["count"]) != int(
            historical_details.get("count", -1)
        ):
            return None
...
def filtered_row_ids(
    ...
    *,
    ...
    if normalized_search:
        # Do not reduce a nullable pandas StringDtype frame with ``any``.
        # Depending on the pandas version, that can raise
        # ``Cannot perform reduction 'any' with string dtype``.  Iterate by
        # position so duplicate Excel headers are safe as well.
        visible = pd.Series(False, index=frame.index, dtype=bool)
        for column_index in range(frame.shape[1]):
            values = frame.iloc[:, column_index].map(excel_filter_text).astype("string")
            matched = values.str.casefold().str.contains(
                re.escape(normalized_search), regex=True, na=False
            )
            visible |= matched.fillna(False).astype(bool)
        visible_mask = visible.tolist()
        row_ids = [
            row_id
...
def _direct_error_header_key(value: object) -> str:
    return re.sub(r"\s+", "", str(value or "").lstrip("\ufeff")).casefold()


def parse_direct_error_order_numbers(
    pasted_text: str,
    available_order_numbers: set[str] | None = None,
) -> tuple[list[str], list[str]]:
    """Read Excel clipboard TSV and return order numbers plus input duplicates.

    The original row itself is never trusted for dashboard dimensions.  It is
    only used to identify a row in the current preprocessed upload.
    """
    ...
    if len(pasted_text) > 1_000_000:
        raise ValueError("붙여넣기 내용이 너무 큽니다. 한 번에 500행까지 등록할 수 있습니다.")
    ...
    if header_index is not None:
        ...
        order_column = next(
            (
                ...
                if _direct_error_header_key(value) in order_aliases
            ),
            None,
        )
        for row in rows[header_index + 1 :]:
            value = row[order_column] if order_column is not None and order_column < len(row) else ""
            order_number = normalize_order_number(value)
            if order_number:
                order_numbers.append(order_number)
    elif available_order_numbers:
        # Excel lets a reviewer copy only the data rows.  In that case, find
        # the one cell per row that actually exists as an order number in the
        # active upload; dates, people and free text are never trusted.
        for row in rows:
            matched = next(
                (
                    normalized
                    for value in row
                    if (normalized := normalize_order_number(value))
                    in available_order_numbers
                ),
                "",
            )
            if matched:
                order_numbers.append(matched)
    else:
        raise ValueError("'오더번호' 열 제목을 포함해 복사하거나 현재 원본 행을 붙여 넣어 주세요.")
    if not order_numbers:
        raise ValueError("붙여넣은 행에 유효한 오더번호가 없습니다.")
    if len(order_numbers) > MAX_DIRECT_ERROR_ROWS:
        raise ValueError(
    ...
    for order_number in order_numbers:
        if order_number in seen and order_number not in duplicates:
...
class CurrentJobManager:
        ...
        with self._lock:
            ...
            for order_number in order_numbers[:30]:
                ...
                rows.append(
                    {
                        "오더번호": order_number,
                        "오더생성일": json_cell_value(
                            "오더생성일", row.get("오더생성일")
                        ),
                        "사업부": json_value(row.get("사업부")),
                        "생성인": json_value(row.get("생성인")),
                        "소분류": json_value(row.get("소분류")),
                        "내역": json_value(row.get("내역")),
                        "상태": state,
                    }
                )
            ready_count = (
    ...
    def approve_direct_error_rows(
        self,
        job_id: str | None,
        pasted_text: str,
    ) -> dict[str, object]:
        """Confirm FN rows pasted from the source Excel without trusting its data."""
        with self._lock:
            job = self._require_job(job_id)
            preview = self.preview_direct_error_rows(job.job_id, pasted_text)
            missing = list(preview["missing_order_numbers"])
            already_confirmed = list(preview["already_confirmed_order_numbers"])
            duplicates = list(preview["duplicate_order_numbers"])
            if duplicates:
                raise ValueError(
                    "붙여넣은 오더번호가 중복되었습니다: "
                    + ", ".join(duplicates[:10])
            ...
            if missing:
                raise ValueError(
                    "현재 업로드 데이터에서 찾지 못한 오더번호가 있습니다: "
                    + ", ".join(missing[:10])
                )
            if already_confirmed:
                raise ValueError(
                    "이미 오생성으로 반영된 오더번호가 있습니다: "
            ...
            for order_number in order_numbers:
                ...
                if pd.isna(order_date):
                    raise ValueError(
                        f"오더번호 {order_number}의 오더생성일이 없어 집계할 수 없습니다."
                    )
                payload = {
                    str(column): json_value(value)
                    for column, value in row.items()
                }
                payload["확정경로"] = "원본 행 직접 등록"
                records.append(
                    {
            ...
            restored_records = [
                record
                for record in records
                if normalize_order_number(record["order_number"]) in excluded_orders
            ]
            new_records = [
                record
                for record in records
                if normalize_order_number(record["order_number"])
                not in excluded_orders
            ]
            restored_order_numbers = sorted(
                {
            ...
            try:
                if new_records:
                    ...
                    result = {
                        ...
                        "approved_count": 0,
                        "approved_at": approved_at,
                        "data_start": min(record["order_date"] for record in records),
                        "data_end": max(record["order_date"] for record in records),
                        "row_ids": [],
                        "order_numbers": [],
                    }
            except Exception:
            ...
            result["restored_count"] = restored_count
            result["restored_order_numbers"] = restored_order_numbers
            result["approved_count"] = int(result["approved_count"]) + restored_count
            result["row_ids"] = [
                int(record["candidate_row_id"]) for record in records
            ]
            result["order_numbers"] = sorted(
                {normalize_order_number(record["order_number"]) for record in records}
            )
            result.update(
                {
                    "registration_type": "direct_source_paste",
                    "candidate_count": len(job.candidates),
                    "checked_count": sum(job.candidate_checked),
                }
            )
    ...
    def rollback_latest(self, job_id: str | None) -> dict[str, object]:
        with self._lock:
            ...
            result = DASHBOARD_STORE.rollback_latest_error_batch(
                # A rollback from the current administrator view must never
                # undo a newer/older approval that belongs to another upload.
    ...
    def _persist_active_views(
        ...
        *,
        ...
        candidate_summary: dict[str, object],
        aggregate_summary: dict[str, object],
        workbook_auto_errors: pd.DataFrame | None = None,
        require_candidate_file: bool = False,
    ) -> None:
        """Commit current-job cache, XLSX and manifest as one visible snapshot."""
        candidate_path = WEB_OUTPUT_ROOT / job.job_id / job.candidate_file
        if require_candidate_file and not candidate_path.is_file():
        ...
        try:
            if candidate_path.is_file():
                temporary_candidate = candidate_path.with_name(
                    f".{candidate_path.stem}_{uuid.uuid4().hex[:8]}_snapshot.xlsx"
                )
                save_classification_workbook(
                    candidates,
                    workbook_auto_errors
                    if workbook_auto_errors is not None
                    else auto_errors,
                    temporary_candidate,
                )
            ...
            if temporary_candidate is not None:
                candidate_backup = candidate_path.with_name(
                    f".{candidate_path.stem}_{uuid.uuid4().hex[:8]}_backup.xlsx"
                )
        ...
        # Publish the in-memory view last: failed disk work cannot leave a
        # newly cached table pointing at an older download/manifest snapshot.
        ...
        job.candidate_checked = list(checked)
        job.candidate_summary = deepcopy(candidate_summary)
        ...
        *,
        ...
        with self._lock:
            ...
            self._persist_active_views(
                job,
                candidates=candidates,
                auto_errors=automatic_source,
                confirmed_errors=confirmed_errors,
                checked=[False] * len(candidates),
                candidate_summary=candidate_summary,
                aggregate_summary=aggregate_summary,
                workbook_auto_errors=active_confirmed,
            )
            confirmed_count = int(
                confirmed_errors["집계상태"].eq("확정").sum()
            )
...
class DirectErrorPasteRequest(BaseModel):
    pasted_text: str = Field(min_length=1, max_length=1_000_000)
    job_id: str | None = None
...
async def analyze(
    ...
    try:
        ...
        LOGGER.exception(
            "Administrator upload analysis failed at %s%% (analysis_id=%s)",
            int((analysis_progress(analysis_id) or {}).get("percent", 0)),
            analysis_id,
        )
        set_analysis_progress(
...
@app.post("/api/admin/direct-errors/preview")
def preview_direct_errors(request: DirectErrorPasteRequest) -> dict[str, object]:
    try:
        return CURRENT_JOB.preview_direct_error_rows(
            request.job_id,
            request.pasted_text,
        )
    except LookupError as error:
...
@app.post("/api/admin/direct-errors/approve")
def approve_direct_errors(
    request: DirectErrorPasteRequest,
    background_tasks: BackgroundTasks,
) -> dict[str, object]:
    analysis_locked = False
    try:
        if not ANALYSIS_LOCK.acquire(blocking=False):
            raise HTTPException(
                status_code=409,
                detail="데이터 분석이 진행 중입니다. 완료 후 직접 등록해 주세요.",
            )
        analysis_locked = True
        try:
            result = CURRENT_JOB.approve_direct_error_rows(
                request.job_id,
                request.pasted_text,
            )
            sync_summary = CURRENT_JOB.sync_active_views(request.job_id)
            ...
            if "result" in locals():
                ...
                try:
                    ...
                    LOGGER.exception(
                        "Failed to restore admin views after direct registration rollback"
                    )
            raise
        result["classification"] = sync_summary
        result.update(
        ...
        LOGGER.exception("Failed to register pasted source rows as errors")
        raise HTTPException(
            status_code=500,
            detail=f"원본 행 직접 등록 및 화면 갱신에 실패했습니다: {error}",
        ) from error
    finally:
        if analysis_locked:
            ANALYSIS_LOCK.release()
    ...
    result["notification"] = EVENTS.publish(
        "new_errors_approved",
        (
            f"원본 행 {result['approved_count']:,}건을 오생성으로 직접 등록했습니다. "
            f"(데이터: {display_date} 기준)"
        ),
        result.copy(),
    )
