...
def mask_personal_data_frame(
    ...
    *,
    ...
    for column in _address_columns(result.columns):
        # Excel imports can expose address columns as object, pandas string,
        # or Arrow-backed string dtype.  Normalize the predicate explicitly;
        # reducing a retained StringDtype with ``any`` raises
        # ``Cannot perform reduction 'any' with string dtype`` on some inputs.
        ...
        current_mask_count = int(mask.sum())
        address_mask_count += current_mask_count
        changed |= mask
