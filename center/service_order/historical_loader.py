...
class HistoricalLoadResult:
    """Historical aggregates plus the confirmed-error rows for monitoring."""
...
def load_historical_error_details(
    truth_path: str | Path,
    except_list_path: str | Path,
    *,
    months: Iterable[int] = range(1, 7),
    allowed_order_numbers: set[str] | None = None,
) -> pd.DataFrame:
    """Return only confirmed 1~6월 rows for the monitoring list.

    The truth workbook already contains the original service-order columns.
    Unlike the total-data loader, this function reads only the small confirmed
    error sheets and never retains a monthly full-data workbook.
    """
    truth_file = Path(truth_path)
    if not truth_file.is_file():
        raise HistoricalInputError(f"정답 파일을 찾을 수 없습니다: {truth_file}")
    exclusion_keys = _load_exclusion_keys(Path(except_list_path))
    frames: list[pd.DataFrame] = []
    seen_orders: set[str] = set()

    for month in tuple(int(value) for value in months):
        ...
        try:
            ...
            raise HistoricalInputError(
                f"정답 파일의 '{sheet_name}' 시트를 읽을 수 없습니다."
            ) from exc
        raw.columns = raw.columns.astype(str).str.strip()
        required = set(TOTAL_COLUMNS)
        missing = sorted(required - set(raw.columns))
        if missing:
            raise HistoricalInputError(
                f"정답 시트 '{sheet_name}'에 필수 열이 없습니다: {missing}"
            )
        raw = raw.dropna(axis=0, how="all").copy()
        raw[ORDER_NUMBER] = _normalize_order_series(raw[ORDER_NUMBER])
        raw[ORDER_DATE] = pd.to_datetime(raw[ORDER_DATE], errors="coerce")
        if raw[ORDER_NUMBER].isna().any() or raw[ORDER_DATE].isna().any():
            raise HistoricalValidationError(
                f"정답 시트 '{sheet_name}'에 오더번호 또는 오더생성일 누락 행이 있습니다."
            )

        department = raw[DEPARTMENT].astype("string").str.strip()
        business = raw[BUSINESS].astype("string").str.strip()
        subcategory = raw[SUBCATEGORY].astype("string").str.strip()
        ...
        keep = (
            ...
            & ~subcategory.map(_subcategory_key).isin(exclusion_keys)
            & ~status.eq(CANCELLED_ORDER_STATUS).fillna(False)
            & raw[ORDER_DATE].dt.month.eq(month)
        )
        details = raw.loc[keep].copy()
        if allowed_order_numbers is not None:
            details = details.loc[
                details[ORDER_NUMBER].astype("string").isin(allowed_order_numbers)
            ].copy()
        details[ORDER_DATE] = details[ORDER_DATE].dt.strftime("%Y-%m-%d")
        duplicate_orders = set(details[ORDER_NUMBER].astype(str)) & seen_orders
        if duplicate_orders:
            sample = ", ".join(sorted(duplicate_orders)[:10])
            raise HistoricalValidationError(f"정답 오더번호가 중복됩니다: {sample}")
        seen_orders.update(details[ORDER_NUMBER].astype(str))
        frames.append(details.reset_index(drop=True))
...
def load_historical_aggregates(
    ...
    *,
    ...
    error_details = load_historical_error_details(
        truth_file,
        except_list_path,
        months=month_values,
        allowed_order_numbers=matched_orders,
    )
    if len(error_details) != summary.truth_count:
        raise HistoricalValidationError(
            "과거 오생성 상세 행과 집계 건수가 일치하지 않습니다: "
            f"상세 {len(error_details):,}건 / 집계 {summary.truth_count:,}건"
        )
