...
def json_cell_value(value: object) -> object:
    """Convert pandas/openpyxl scalars before persisting a JSON payload."""
    if value is None:
        return None
    try:
        if pd.isna(value):
...
class DashboardStore:
    ...
    def _initialize(self) -> None:
        with self._lock, self._connection() as connection:
            ...
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS historical_error_details (
                ...
                CREATE INDEX IF NOT EXISTS idx_historical_error_details_period
                    ON historical_error_details (order_date, business, person);
    ...
    def replace_historical_error_details(
        self,
        ...
        *,
        ...
        """Replace the retained 1~6월 confirmed-error rows used by monitoring.

        This table deliberately contains only confirmed error rows; full
        historical uploads remain aggregate-only and are discarded.
        """
        required = {"오더번호", "오더생성일", "생성인", "사업부", "소분류"}
        ...
        if missing:
            raise KeyError(f"Missing historical error columns: {missing}")
        ...
        for index, row in details.iterrows():
            ...
            if not order_number or order_number in seen_orders:
                raise ValueError(
                    f"Historical error order number is empty or duplicated at row {index}."
                )
            seen_orders.add(order_number)
            order_date = self._coerce_aggregate_date(
                row.get("오더생성일"), f"오더생성일 at row {index}"
            ).isoformat()
            payload = mask_payload(
                {
                    str(column): json_cell_value(value)
                    for column, value in row.items()
                }
            )
            payload["오더번호"] = order_number
            payload["오더생성일"] = order_date
            records.append(
                (
                    order_number,
                    order_date,
                    clean_dimension_value(row.get("생성인")),
                    clean_dimension_value(row.get("사업부")),
                    clean_dimension_value(row.get("소분류")),
        ...
        with self._lock, self._connection() as connection:
            connection.execute("BEGIN IMMEDIATE")
            connection.execute("DELETE FROM historical_error_details")
            if records:
                connection.executemany(
                    """
                    INSERT INTO historical_error_details (
                        order_number, order_date, person, business, subcategory,
                        payload_json, source_name, imported_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    records,
                )
    ...
    def historical_error_detail_status(self) -> dict[str, object]:
        with self._lock, self._connection() as connection:
            row = connection.execute(
                """
                SELECT COUNT(*) AS count,
                       MIN(order_date) AS start,
                       MAX(order_date) AS end
                FROM historical_error_details
                """
            ).fetchone()
        return {
            "count": int(row["count"]),
    ...
    @staticmethod
    def _historical_error_where(
        start_date: date,
        end_date: date,
        person: str | None,
        business: str | None,
    ) -> tuple[str, list[object]]:
        clauses = ["h.order_date BETWEEN ? AND ?"]
        parameters: list[object] = [start_date.isoformat(), end_date.isoformat()]
        if person:
            clauses.append("h.person = ?")
            parameters.append(person)
        if business:
            clauses.append("h.business = ?")
            parameters.append(business)
        return " AND ".join(clauses), parameters
    ...
    def new_error_summary(
        ...
        *,
        ...
        historical_where, historical_parameters = self._historical_error_where(
            start_date, end_date, person, business
        )
        with self._lock, self._connection() as connection:
            row = connection.execute(
                f"""
                SELECT COUNT(*) AS count,
                       MIN(event_at) AS since_date,
                       MAX(event_at) AS last_updated,
                       MAX(order_date) AS as_of_date
                FROM (
                    SELECT d.order_date AS order_date, b.approved_at AS event_at
                    FROM error_approval_details AS d
                    JOIN error_approval_batches AS b ON b.batch_id = d.batch_id
                    WHERE {where}
                    UNION ALL
                    SELECT h.order_date AS order_date, h.imported_at AS event_at
                    FROM historical_error_details AS h
                    WHERE {historical_where}
                )
                """,
    ...
    def new_error_details(
        ...
        *,
        ...
        with self._lock, self._connection() as connection:
            rows = connection.execute(
                f"""
                SELECT payload_json, order_date, event_at
                FROM (
                    SELECT d.payload_json AS payload_json,
                           d.order_date AS order_date,
                           b.approved_at AS event_at,
                           d.detail_id AS stable_id
                    FROM error_approval_details AS d
                    JOIN error_approval_batches AS b ON b.batch_id = d.batch_id
                    WHERE {where}
                    UNION ALL
                    SELECT h.payload_json AS payload_json,
                           h.order_date AS order_date,
                           h.imported_at AS event_at,
                           0 AS stable_id
                    FROM historical_error_details AS h
                    WHERE {historical_where}
                )
                ORDER BY order_date, event_at, stable_id
                """,
                [*parameters, *historical_parameters],
            ).fetchall()
        ...
        approved_times = [row["event_at"] for row in rows]
    ...
    def overview(
        ...
        *,
        ...
        # Monitoring rows include retained 1~6월 confirmed details as well as
        # approvals from later uploads.  The old in-query row covered only the
        # latter, so its count could disagree with the downloadable list.
        new_error_row = self.new_error_summary(
