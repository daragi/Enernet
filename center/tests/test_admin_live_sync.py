...
class AdminLiveSyncTest(TestCase):
    ...
    def test_rollback_is_scoped_to_the_current_upload_job(self) -> None:
        """A later approval from another upload must not be rolled back here."""
        with TemporaryDirectory() as directory:
            root = Path(directory)
            store = DashboardStore(root / "metrics.sqlite3")
            manager = server.CurrentJobManager()
            ...
            other = sample_orders()
            other["오더번호"] = "90000002"
            other["오더생성일"] = pd.Timestamp("2026-07-17")
            all_orders = pd.concat([current, other], ignore_index=True)
            candidates = format_classified_orders(current)
            empty = format_classified_orders(current.iloc[0:0])
            store.replace_totals(all_orders)
            ...
            with (
                ...
                current_approval = manager.approve_rows("current-job", [0])
                store.approve_error_batch(
                    batch_id="other-approval",
                    job_id="other-job",
                    source_name="other.xlsx",
                    approved_at="2026-07-30T12:01:00+09:00",
                    records=[
                        {
                            "candidate_row_id": 0,
                            "order_number": "90000002",
                            "order_date": "2026-07-17",
                            "person": "테스트",
                            "business": "북부",
                            "subcategory": "검침",
                            "payload": other.iloc[0].to_dict(),
                        }
                    ],
                )

                rolled_back = manager.rollback_latest("current-job")

            self.assertEqual(
                rolled_back["batch_id"], current_approval["batch_id"]
            )
            self.assertEqual(
                store.period_status("2026-07-16", "2026-07-16")[
                    "error_count"
            ...
            self.assertEqual(
                store.period_status("2026-07-17", "2026-07-17")[
                    "error_count"
                ],
                1,
            )

    def test_pasted_source_row_registers_fn_as_confirmed_error(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            store = DashboardStore(root / "metrics.sqlite3")
            manager = server.CurrentJobManager()
            ...
            with (
                ...
                preview = manager.preview_direct_error_rows("d1ec7e900001", pasted)
                headerless_preview = manager.preview_direct_error_rows(
                    "d1ec7e900001",
                    "H071\t90000001\t처리완료\t검침\t확정 누락 테스트\n",
                )
                approved = manager.approve_direct_error_rows("d1ec7e900001", pasted)
                synced = manager.sync_active_views("d1ec7e900001")

                self.assertEqual(preview["ready_count"], 1)
                self.assertEqual(preview["missing_order_numbers"], [])
                self.assertEqual(headerless_preview["ready_count"], 1)
                self.assertEqual(approved["approved_count"], 1)
                self.assertEqual(
                    approved["registration_type"], "direct_source_paste"
                )
                self.assertEqual(synced["confirmed_error_count"], 1)
                self.assertEqual(
                    store.period_status("2026-07-16", "2026-07-16")[
                        "error_count"
                    ],
                ...
                rolled_back = manager.rollback_latest("d1ec7e900001")
                restored = manager.sync_active_views(
                    "d1ec7e900001",
                    restore_order_numbers=rolled_back["order_numbers"],
                )

            self.assertEqual(rolled_back["rolled_back_count"], 1)
            self.assertEqual(restored["confirmed_error_count"], 0)
            self.assertEqual(
                store.period_status("2026-07-16", "2026-07-16")[
                    "error_count"
                ],
