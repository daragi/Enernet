...
def test_string_dtype_address_columns_are_fully_masked() -> None:
    source = pd.DataFrame(
        {
            "주소": pd.Series(["서울시 중구 1", pd.NA], dtype="string"),
            "구주소": pd.Series(["중구동 2", ""], dtype="string"),
        }
    )

    masked, summary = mask_personal_data_frame(source)

    assert masked["주소"].tolist() == ["***", pd.NA]
    assert masked["구주소"].tolist() == ["***", ""]
    assert summary["주소마스킹건수"] == 2
...
def test_historical_error_details_follow_the_selected_month() -> None:
    historical = pd.DataFrame(
        {
            ...
            "사업부": ["중부"],
            "소분류": ["검침"],
            "내역": ["전화번호 010-1234-5678"],
            "주소": ["서울시 테스트 주소"],
        }
    )
    with TemporaryDirectory() as directory:
        store = DashboardStore(Path(directory) / "metrics.sqlite3")
        stored = store.replace_historical_error_details(
            historical,
            source_name="historical.xlsx",
            imported_at="2026-07-31T09:00:00+09:00",
        )
        june, june_summary = store.new_error_details(
            time_mode="month",
            year=2026,
            month=6,
            start=None,
        ...
        july, _ = store.new_error_details(
            time_mode="month",
            year=2026,
            month=7,
            start=None,
    ...
    assert june_summary["count"] == 1
    assert june["오더번호"].tolist() == ["HIST-06"]
    assert june.loc[0, "주소"] == "***"
    assert "010-" not in june.loc[0, "내역"]
    assert july.empty
...
def test_global_search_handles_nullable_string_columns() -> None:
    """Global search must not reduce pandas StringDtype values with any()."""
    frame = pd.DataFrame(
        {
            "소분류": pd.Series(["검침", "체납"], dtype="string"),
            "내역": pd.Series(["계량기 확인", "예고장 송달"], dtype="string"),
        }
    )

    assert filtered_row_ids(
        frame,
        search="예고장",
        column_filters=None,
    ) == [1]
