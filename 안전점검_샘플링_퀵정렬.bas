Attribute VB_Name = "SamplingModule_Quick"
'-----------------------------------------------------------------
' Safety Inspection Sampling Macro (Optimized with Quick Sort & Speedups)
...
' - Performance: ScreenUpdating disabled, Quick Sort algorithm applied
'-----------------------------------------------------------------
...
Sub SamplingInspector()
    ...
    '--- (Speedup 1) Enable Performance Settings ---
    ...
    If hasY Then
        ' Temporarily restore screen updating to show MsgBox properly
    ...
    '--- 9. Sort Selected Rows in Ascending Order (Optimized with Quick Sort) ---
    ...
    If selectedCount > 1 Then
        Call QuickSort(finalSelectedRows, 1, selectedCount)
    End If
...
'-----------------------------------------------------------------
Private Sub QuickSort(ByRef arr() As Long, ByVal left As Long, ByVal right As Long)
